...
function applyCornerSnaps(
  ...
  const moveCorner = openChain
    ? (pts: OrthoPoint[], idx: number, x: number, y: number) =>
        moveCorner2DOpen(pts, idx, x, y, openPin, portFrame)
    : moveCorner2D;
  ...
  useEffect(() => {
    const onMove = (e: PointerEvent) => {
      ...
      if (st.mode === "corner") {
        ...
        next = snapped.points;
        if (isSeg) {
          next = enforcePortStubElbow(next, pin, frame);
        }
      ...
      if (movedSeg && !movedSeg.horizontal) {
        ...
        if (Math.abs(snappedX - movedSeg.a.x) > 0.5) {
          ...
          if (!isSeg) {
            ...
            // Always clamp against the port / machine — never snap into the body.
            x = clampOpenVerticalX(
              x,
              next,
              st.segmentIndex,
              pin,
      ...
      // Straight bus V: stubs follow the sliding column live (T stays a T).
      if (
        isSeg &&
        st.mode === "segment" &&
        st.translateRail &&
